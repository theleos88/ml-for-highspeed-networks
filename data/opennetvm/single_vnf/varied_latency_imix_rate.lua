...
function master(args)
	...
	mg.startTask("loadSlave", txDev:getTxQueue(0), rxDev, 60, 2)
...
function loadSlave(queue, rxDev, size, flows)
	...
	while mg.running() do
		bufs:alloc(size)
        	...
        	if limiter:expired()
        	then
			local interval = 2*r + math.random(1,10)
            		print( "Next expired time", interval)
            		limiter:reset(interval)

            		if r <= 10
            		then 
                        	rate = math.random(500, 10000)
