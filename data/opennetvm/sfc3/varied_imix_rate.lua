...
function master(args)
	...
	mg.startTask("loadSlave", txDev:getTxQueue(0), rxDev, 60, 3)
	mg.startTask("timerSlave", txDev:getTxQueue(1), rxDev:getRxQueue(1), 124, 3)
	mg.waitForTasks()
end
...
function loadSlave(queue, rxDev, size, flows)
	...
	local sizes = {60, 60, 60, 60, 60, 60, 60, 566, 566, 566, 566, 1510}
        local limiter = timer:new(100)

        local rates = {500, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 9000, 8000, 7000, 6000, 5000, 4000, 3000, 2000, 1000, 500} 
        local r = 2
        local rate=rates[1]
	...
	while mg.running() do
		...
		for i, buf in ipairs(bufs) do
			...
			counter = incAndWrap(counter, flows) end
